...
export async function validate(diagnosticCollection: vscode.DiagnosticCollection, doc: vscode.TextDocument, srcArray) {
    ...
    v.on('hmr:status', function (params) {
        if (params.violations) {
            ...
            params.violations.forEach(violation => {
